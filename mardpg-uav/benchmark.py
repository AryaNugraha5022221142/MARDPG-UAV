...
Usage:
    ...
    python benchmark.py --episodes 200 --device cuda
"""
...
def main():
    ...
    buf = SequenceReplayBuffer(capacity=100_000, seq_len=seq,
    ...
    for ep in range(args.episodes):
        ...
        for _ in range(stage_cfg['max_steps']):
            ...
            buf.add_transition(obs.copy(), prev_actions.copy(),
                               actions, rewards, next_obs.copy(),
        ...
        # One update batch every 5 episodes (mirrors train.py's update path)
        if len(buf) >= b_sz and ep % 5 == 0:
            batch = buf.sample(b_sz)
            if batch is not None:
                t0 = time.perf_counter()
                (b_obs, b_nobs, b_prev_act, b_act,
                 b_rew, b_done, b_pad) = [b.to(device) for b in batch]
                ...
                with torch.no_grad():
                    nf = agents[0].actor_target.shared(
                        b_nobs.permute(0, 2, 1, 3)
                        .reshape(b_sz * n_agents * seq, obs_dim)
                ...
                burn_mask  = (torch.arange(seq, device=device)
                              .view(1, -1, 1) >= bi)
                alive_prev = ~torch.cat([
                    torch.zeros(b_sz, 1, n_agents, dtype=torch.bool, device=device),
                    b_done[:, :-1, :]], dim=1)
                amask = burn_mask & alive_prev & (~b_pad).unsqueeze(-1)

                for i, ag in enumerate(agents):
                    ag.critic_optimizer.zero_grad()
                    cl_i, _, valid_steps = ag.compute_critic_loss(
    ...
    bottleneck = ("ENV (GPU won't help much — raise grad_steps_per_update "
                  "and/or parallelise envs)" if env_time > update_time
                  else "UPDATE (GPU will help)")
