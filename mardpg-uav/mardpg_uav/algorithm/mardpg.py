...
class MARDPGAgent:
    ...
    def __init__(self, agent_id: int, n_agents: int, obs_dim: int=35, action_dim: int=2, action_bound: float=0.5236, lstm_hidden: int=128, fc_hidden: int=128, lr_actor: float=0.001, lr_critic: float=0.001, tau: float=0.01, gamma: float=0.99, gradient_clip: float=1.0, burn_in: int=10, huber_beta: float=10.0, twin_critic: bool=False, recurrent: bool=True, centralized: bool=True, device: str='cpu'):
        ...
        self.actor = Actor(self.shared_extractor, lstm_hidden, max_delta_angle=action_bound, action_dim=action_dim, recurrent=recurrent).to(self.device)
        self.actor_target = copy.deepcopy(self.actor).to(self.device)
        self.actor_target.eval()
        n_critic = n_agents if centralized else 1
        self.critic = RecurrentCritic(n_critic, obs_dim, action_dim, fc_hidden, lstm_hidden, recurrent=recurrent).to(self.device)
        self.critic_target = copy.deepcopy(self.critic).to(self.device)
        self.critic_target.eval()
        if self.twin_critic:
            self.critic2 = RecurrentCritic(n_critic, obs_dim, action_dim, fc_hidden, lstm_hidden, recurrent=recurrent).to(self.device)
            self.critic2_target = copy.deepcopy(self.critic2).to(self.device)
            self.critic2_target.eval()
            self.critic2_optimizer = optim.Adam(self.critic2.parameters(), lr=lr_critic)
        self.actor_private_params = list(self.actor.lstm.parameters()) + list(self.actor.fc_out.parameters())
    ...
    def compute_critic_loss(self, obs_all_seq, act_all_seq, next_obs_all_seq, next_act_all_seq, rewards, dones, mask):
        """
        ...
        (q1, _) = self.critic(c_obs, c_act, hidden=None, seq_len=total_seq_len)
        q1 = q1.view(batch_size, total_seq_len)
        if self.twin_critic:
            (q2, _) = self.critic2(c_obs, c_act, hidden=None, seq_len=total_seq_len)
            q2 = q2.view(batch_size, total_seq_len)
        with torch.no_grad():
            (c_next_obs, c_next_act) = self._critic_inputs(next_obs_all_seq, next_act_all_seq)
            (q1_next, _) = self.critic_target(c_next_obs, c_next_act, hidden=None, seq_len=total_seq_len)
            q1_next = q1_next.view(batch_size, total_seq_len)
            if self.twin_critic:
                (q2_next, _) = self.critic2_target(c_next_obs, c_next_act, hidden=None, seq_len=total_seq_len)
    ...
    def compute_actor_loss(self, obs_all_seq: torch.Tensor, act_all_seq: torch.Tensor, prev_act_all_seq: torch.Tensor, mask: torch.Tensor):
        ...
        stored_my = act_all_seq.view(batch_size, total_seq_len, self.n_agents, -1)[:, :, self.agent_id, :].detach()
        t_idx = torch.arange(total_seq_len, device=my_acts.device).view(1, -1, 1)
        mixed_my = torch.where(t_idx >= learn_start, my_acts, stored_my)
        ...
        (q_flat, _) = self.critic(c_obs, c_joint_acts, hidden=None, seq_len=total_seq_len)
