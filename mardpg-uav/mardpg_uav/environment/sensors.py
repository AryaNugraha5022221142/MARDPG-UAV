...
class Rangefinder:
    ...
    def __init__(self, range_max: float=10.0, seed: int=None):
        ...
        pitch_grid = np.broadcast_to(pitch_angles[:, None], (self.n_v, self.n_h)).flatten()
        self._dir_vecs = np.stack([np.cos(pitch_grid) * np.cos(yaw_grid), np.cos(pitch_grid) * np.sin(yaw_grid), np.sin(pitch_grid)], axis=-1).astype(np.float32)

    def scan(self, position: np.ndarray, theta: float, phi: float, obstacles: List[Obstacle], sigma_l: float=0.02, obs_centers: np.ndarray=None, obs_max_sizes: np.ndarray=None, extra_obstacles: List[Obstacle]=None) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        if boxes:
            c = np.array([b.position for b in boxes], dtype=np.float32)[:, None, :]
            s = np.array([b.size for b in boxes], dtype=np.float32)[:, None, :]
            d = rotated_dirs[None, :, :]
        ...
        if sigma_l > 0.0:
            ...
            distances = np.where(distances < self.range_max - 0.001, distances + noise * self.range_max, distances)
