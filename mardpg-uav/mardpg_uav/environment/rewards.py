...
class RewardFunction:
    ...
    def __init__(self, alpha: float=3.0, lambda_col: float=5.0, sigma_col: float=15.0, sigma_col_uav: float=None, r_free: float=0.1, r_step: float=-0.6, delta: Tuple[float, float, float, float]=(0.45, 0.3, 0.15, 0.1), collision_radius: float=0.5, inter_uav_min: float=1.0, range_max: float=10.0):
        self.alpha = alpha
    ...
    def compute(self, agent_id: int, position: np.ndarray, goal: np.ndarray, rangefinder_raw: np.ndarray, rangefinder_norm: np.ndarray, other_positions: List[np.ndarray], obstacles: List, obs_centers: np.ndarray=None, obs_max_sizes: np.ndarray=None) -> float:
        """
        ...
        d_uav = min((np.linalg.norm(position - p) for p in other_positions), default=float('inf'))
    ...
    def _surface_distance(self, position, obs):
        ...
        if obs.type == 'cylinder':
            (r, h) = obs.size
            q = np.array([np.linalg.norm(p[:2] - c[:2]) - r, abs(p[2] - c[2]) - h / 2.0])
