...
class MultiUAVEnv(gym.Env):
    ...
    def __init__(self, config: dict):
        ...
        self.dynamics = QuadcopterDynamics(v=config.get('v', 1.0), dt=config['dt'], env_size=config['env_size'], max_altitude=config['max_altitude'], min_altitude=config.get('min_altitude', 0.0), max_delta_angle=config.get('max_delta_angle', np.pi / 6))
        self.scene_gen = SceneGenerator(seed=config.get('seed', None))
        self.rangefinder = Rangefinder(range_max=config['sensor_range'], seed=config.get('seed', None))
        self.reward_fn = RewardFunction(alpha=config['reward']['alpha'], lambda_col=config['reward']['lambda_col'], sigma_col=config['reward']['sigma_col'], sigma_col_uav=config['reward'].get('sigma_col_uav', config['reward']['sigma_col']), r_free=config['reward']['r_free'], r_step=config['reward']['r_step'], delta=tuple(config['reward']['delta']), collision_radius=config['collision_radius'], inter_uav_min=config['inter_uav_min_dist'], range_max=config['sensor_range'])
        self.uav_sense_radius = float(config.get('uav_sense_radius', config.get('inter_uav_min_dist', 1.0)))
        self.obs_dim = 4 + 25 + 5 + 1
        self.action_dim = config.get('action_dim', 2)
        self.observation_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(self.n_agents, self.obs_dim), dtype=np.float32)
        self.action_space = gym.spaces.Box(low=-self.cfg.get('max_delta_angle', np.pi / 6), high=self.cfg.get('max_delta_angle', np.pi / 6), shape=(self.n_agents, self.action_dim), dtype=np.float32)
    ...
    def reset(self, stage_cfg: dict=None) -> np.ndarray:
        ...
        walls = [Obstacle('box', np.array([-t, ey / 2, ez / 2]), np.array([t, ey / 2, ez / 2])), Obstacle('box', np.array([ex + t, ey / 2, ez / 2]), np.array([t, ey / 2, ez / 2])), Obstacle('box', np.array([ex / 2, -t, ez / 2]), np.array([ex / 2, t, ez / 2])), Obstacle('box', np.array([ex / 2, ey + t, ez / 2]), np.array([ex / 2, t, ez / 2])), Obstacle('box', np.array([ex / 2, ey / 2, min_z - t]), np.array([ex / 2, ey / 2, t])), Obstacle('box', np.array([ex / 2, ey / 2, ez + t]), np.array([ex / 2, ey / 2, t]))]
        ...
        for i in range(self.n_agents):
            self.agents_state[i, 3] = self.scene_gen.rng.uniform(-np.pi, np.pi)
            self.agents_state[i, 4] = self.scene_gen.rng.uniform(-np.pi / 6, np.pi / 6)
    ...
    def _world_to_body(self, vec: np.ndarray, theta: float, phi: float) -> np.ndarray:
        """Rotate a world-frame vector into the UAV body frame.
        ...
        Rt = np.array([[cy * cp, sy * cp, sp], [-sy, cy, 0.0], [-cy * sp, -sy * sp, cp]], dtype=np.float32)
    ...
    def _other_uav_obstacles(self, i: int):
        ...
        return [Obstacle('sphere', self.agents_state[j, :3].copy(), np.array([r], dtype=np.float32)) for j in range(self.n_agents) if j != i and (not done[j])]
    ...
    def step(self, actions: np.ndarray) -> Tuple[np.ndarray, np.ndarray, bool, Dict]:
        ...
        Returns:
            ...
            info: dict
        """
        actions = np.clip(np.array(actions, dtype=np.float32), -self.cfg.get('max_delta_angle', np.pi / 6), self.cfg.get('max_delta_angle', np.pi / 6))
        ...
        positions_arr = self.agents_state[:, :3]
        diffs = positions_arr[:, None, :] - positions_arr[None, :, :]
        dist_matrix = np.linalg.norm(diffs, axis=-1)
        np.fill_diagonal(dist_matrix, np.inf)
        ghost = self.agent_done
        dist_for_collision = dist_matrix.copy()
        dist_for_collision[:, ghost] = np.inf
        inter_collisions = np.any(dist_for_collision < self.cfg['inter_uav_min_dist'], axis=1)
        ...
        for i in range(self.n_agents):
            ...
            (rangefinder_raw, rangefinder_norm) = self.rangefinder.scan(pos, theta, phi, self.obstacles, sigma_l=getattr(self, 'lidar_noise', 0.02), obs_centers=self.obs_centers, obs_max_sizes=self.obs_max_sizes, extra_obstacles=self._other_uav_obstacles(i))
            ...
            rewards[i] = self.reward_fn.compute(i, pos, self.goals[i], rangefinder_raw, rangefinder_norm, other_pos, self.obstacles, obs_centers=self.obs_centers, obs_max_sizes=self.obs_max_sizes)
            if np.linalg.norm(pos - self.goals[i]) < self.cfg['goal_threshold']:
                reached[i] = True
                rewards[i] += self.cfg['reward'].get('r_goal', 10.0)
            elif collisions[i]:
        ...
        info = {'collisions': self.agent_collided.copy(), 'dyn_collisions': dyn_collisions_copy, 'reached': self.agent_reached.copy(), 'safe_inter_uav_ratio': self.safe_inter_uav_steps / max(1, self.steps), 'min_pair_dist': float(self.min_pair_dist) if np.isfinite(self.min_pair_dist) else float('nan'), 'near_miss_ratio': self.near_miss_steps / max(1, self.steps), 'uav_collisions': self.agent_uav_collided.copy(), 'step_collisions': collisions.copy(), 'step_reached': reached.copy(), 'trapped': trapped_agents, 'timeout': timeout, 'steps': self.steps, 'agent_done': self.agent_done.copy()}
    ...
    def _get_single_observation(self, i: int, lidar_norm: np.ndarray=None) -> np.ndarray:
        ...
        if lidar_norm is None:
            (_, lidar_norm) = self.rangefinder.scan(pos, theta, phi, self.obstacles, sigma_l=getattr(self, 'lidar_noise', 0.02), obs_centers=self.obs_centers, obs_max_sizes=self.obs_max_sizes, extra_obstacles=self._other_uav_obstacles(i))
        ...
        goal_block = np.array([d5_norm, np.sin(varpi), np.cos(varpi), np.sin(varpi_z), np.cos(varpi_z)], dtype=np.float32)
        done = getattr(self, 'agent_done', np.zeros(self.n_agents, dtype=bool))
        alive = 1.0 if not done[i] else 0.0
        obs = np.concatenate([[np.sin(theta), np.cos(theta), np.sin(phi), np.cos(phi)], lidar_norm.flatten(), goal_block, [alive]]).astype(np.float32)
    ...
    def _sample_free_position(self) -> np.ndarray:
        ...
        warnings.warn(f'Could not find free position after all attempts. Arena may be over-saturated (obstacle_count={n_obs}).')
        return self.scene_gen.rng.uniform([5.0, 5.0, 5.0], env - 5.0).astype(np.float32)
