...
def plot_scene(ax, env, title):
    for ob in env.obstacles[:-6]:
        if ob.type == 'sphere':
            ...
            for zz in z:
                ax.plot_surface(xx, yy, np.full_like(xx, zz), color='#555555', alpha=1.0, shade=True)
            (xx, zz) = np.meshgrid(x, z)
            for yy in y:
                ax.plot_surface(xx, np.full_like(xx, yy), zz, color='#555555', alpha=1.0, shade=True)
            (yy, zz) = np.meshgrid(y, z)
            for xx in x:
                ax.plot_surface(np.full_like(yy, xx), yy, zz, color='#555555', alpha=1.0, shade=True)
