...
def _draw_box(ax, pos, half, color=OBSTACLE_GRAY, alpha=0.3):
...
def _alpha_ramp_colors(color, n, a0=0.2, a1=1.0):
    rgb = to_rgb(color)
    return np.array([(rgb, rgb, rgb, a) for a in np.linspace(a0, a1, n)])

def _load_agents(checkpoint_dir, env, env_cfg, net_cfg, algo_cfg, device):
    n_agents = env_cfg['n_agents']
    agents = []
    for i in range(n_agents):
        ag = MARDPGAgent(agent_id=i, n_agents=n_agents, obs_dim=env.obs_dim, action_dim=env.action_dim, action_bound=env_cfg.get('max_delta_angle', 0.5236), lstm_hidden=net_cfg.get('actor_lstm_hidden', 128), fc_hidden=net_cfg.get('critic_lstm_hidden', 128), tau=algo_cfg['tau'], gamma=algo_cfg['gamma'], burn_in=algo_cfg['burn_in'], device=device)
        try:
...
def plot_static(env, env_cfg, path, dyn_path, dyn_r, reached, collided, goals, stage_name, out_path):
    ...
    for i in range(n_agents):
        ...
        axtop.scatter(path[0, i, 0], path[0, i, 1], color=c, marker='s', s=45, edgecolor='k', linewidth=0.5, label=f'Agent {i + 1}')
    ...
    axsep.set_xlabel('step')
    axsep.set_ylabel('min pairwise distance (m)')
    axsep.set_title('Swarm separation — dips toward the floor = near-misses')
    axsep.legend(fontsize=8)
    axsep.grid(alpha=0.3)
    for i in range(n_agents):
        c = AGENT_COLORS[i % len(AGENT_COLORS)]
    ...
    axgoal.set_xlabel('step')
    axgoal.set_ylabel('distance to goal (m)')
    axgoal.set_title('Progress — flat tail = trapped; sharp stop = reached/collided')
    axgoal.legend(fontsize=8)
    axgoal.grid(alpha=0.3)
    n_reached = int(reached.sum())
    n_col = int(collided.sum())
...
def plot_trajectory_top_down(env, env_cfg, render, title, out_path):
    ...
    for ob in env.obstacles[:-6]:
        ...
        if ob.type == 'cylinder':
            ...
            box = plt.Rectangle((ob.position[0] - ob.size[0], ob.position[1] - ob.size[1]), ob.size[0] * 2, ob.size[1] * 2, color=c, alpha=0.55, linewidth=0)
            axtop.add_patch(box)
    for i in range(n_agents):
        c = AGENT_COLORS[i % len(AGENT_COLORS)]
...
def plot_trajectory_3d(env, env_cfg, render, title, out_path, elev=22, azim=-58):
    ...
    for i in range(n_agents):
        ...
        agent_handles.append(Line2D([0], [0], color=c, lw=2.6, marker=em, markersize=9, markeredgecolor='k', label=f'Agent {i + 1} — {status}'))
    ax.set_xlim(0, ex)
    ax.set_ylim(0, ey)
    ax.set_zlim(0, ez)
    ...
    leg1 = ax.legend(handles=agent_handles, loc='upper left', bbox_to_anchor=(-0.02, 1.0), fontsize=9, framealpha=0.92, title='Agent (color = identity)')
    ax.add_artist(leg1)
    sem = [Line2D([0], [0], color='0.35', marker='s', ls='None', markersize=9, markeredgecolor='k', label='start'), Line2D([0], [0], color='0.35', marker='*', ls='None', markersize=13, markeredgecolor='k', label='goal'), Line2D([0], [0], color='0.35', marker='o', ls='None', markersize=9, markeredgecolor='k', label='end (reached)'), Line2D([0], [0], color='0.35', marker='X', ls='None', markersize=9, markeredgecolor='k', label='end (collided)'), Line2D([0], [0], color='0.7', lw=6, alpha=0.5, label='obstacle')]
    if dyn_path is not None:
        sem.append(Line2D([0], [0], color=HAZARD_COLOR, lw=1.8, ls='--', label='dynamic threat path'))
    ax.legend(handles=sem, loc='upper right', bbox_to_anchor=(1.02, 1.0), fontsize=9, framealpha=0.92, title='Markers')
    fig.text(0.5, 0.015, 'Line opacity increases start → end (direction of travel).  Dotted line = ground (X–Y) projection.', ha='center', fontsize=9, color='0.3')
...
def animate(env, env_cfg, path, dyn_path, dyn_r, goals, stage_name, out_path, tail=40):
    ...
    for i in range(n_agents):
        ax.scatter(*goals[i], color=AGENT_COLORS[i % len(AGENT_COLORS)], marker='*', s=200, edgecolor='k', linewidth=0.5)
    lines = [ax.plot([], [], [], color=AGENT_COLORS[i % len(AGENT_COLORS)], lw=2) for i in range(n_agents)]
...
def main():
    ...
    p.add_argument('--stage', type=int, default=len(CURRICULUM), help='Curriculum stage scene to evaluate in (1..N). Use the stage the checkpoint was actually trained to.')
    ...
    plot_static(env, env_cfg, path, dyn_path, dyn_r, reached, collided, goals, stage_name, args.out)
    if args.animate:
        animate(env, env_cfg, path, dyn_path, dyn_r, goals, stage_name, args.out.replace('.png', '.mp4'))
